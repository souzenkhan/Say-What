...
type ConnectionState =
  ...
  | "live"
  | "paused"
  | "error"; //allowed connection states
...
const ConnectionContext = createContext<ConnectionContextType | undefined>(
  undefined,
); //create context (starts undefined)
...
export function ConnectionProvider({
  ...
  const [connectionState, setConnectionState] =
    useState<ConnectionState>("idle"); //state + setter (default = idle)

  return (
    <ConnectionContext.Provider value={{ connectionState, setConnectionState }}>
      {" "}
      {/* pass state + setter to context */}
...
export function useConnection() {
  ...
  if (!context) {
    //check if used outside provider
